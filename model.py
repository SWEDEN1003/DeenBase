...
class DEE_module(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        self.FC11 = nn.Conv2d(channel, 1, kernel_size=3, stride=1, padding=1, bias=False, dilation=1)
        self.FC11.apply(weights_init_kaiming)
        self.FC12 = nn.Conv2d(channel, 1, kernel_size=3, stride=1, padding=2, bias=False, dilation=2)
        self.FC12.apply(weights_init_kaiming)
        self.FC13 = nn.Conv2d(channel, 1, kernel_size=3, stride=1, padding=3, bias=False, dilation=3)
        self.FC13.apply(weights_init_kaiming)
        self.FC1 = nn.Conv2d(channel, channel, kernel_size=1)
        self.FC1.apply(weights_init_kaiming)
        
        

        self.FC21 = nn.Conv2d(channel, 1, kernel_size=3, stride=1, padding=1, bias=False, dilation=1)
        self.FC21.apply(weights_init_kaiming)
        self.FC22 = nn.Conv2d(channel, 1, kernel_size=3, stride=1, padding=2, bias=False, dilation=2)
        self.FC22.apply(weights_init_kaiming)
        self.FC23 = nn.Conv2d(channel, 1, kernel_size=3, stride=1, padding=3, bias=False, dilation=3)
        self.FC23.apply(weights_init_kaiming)
        self.FC2 = nn.Conv2d(channel, channel, kernel_size=1)
    ...
    def forward(self, x):
        ...
        att11 = self.FC11(x).sigmoid()
        att12 = self.FC12(x).sigmoid()
        att13 = self.FC13(x).sigmoid()
        
        att21 = self.FC21(x).sigmoid()
        att22 = self.FC22(x).sigmoid()
        att23 = self.FC23(x).sigmoid()
        
        
        x1 = (att11 * x + att12 * x + att13 * x) / 3  
        x1 = self.FC1(F.relu(x1))    
        x2 = (att21 * x + att22 * x + att23 * x) / 3     
